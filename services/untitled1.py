...
def generar_pdf_comodato(nuevo, accesorios_seleccionados):
   ...
   # c.drawImage("heineken_logo.png", x_logo, y_logo, width=logo_width, height=logo_height,
    ...
    # Texto hasta la tercera cláusula
...
"""
    ...
    # ---- Nueva página después de la tercera cláusula ----
    c.showPage()

    # Continuación del contrato (CUARTA en adelante)
...
"""
    ...
    # Encabezados y valores (ya sin RITM)
    ...
    valores = [
        ...
        nuevo.get("Marca", ""),
        nuevo.get("Tipo", ""),
        nuevo.get("IMEI_HOST", ""),       # ✅ CORRECCIÓN CLAVE
        nuevo.get("LINEA", ""),      # ✅
        nuevo.get("LOCALIDAD", "")   # ✅
     ]


    # Convertir valores a Paragraph para que se ajusten
    valores_paragraphs = [Paragraph(str(v), styleN) for v in valores]

    data = [encabezados, valores_paragraphs]

    # Definir anchos más amplios para columnas largas
    ...
    for categoria, seleccionados in accesorios_seleccionados.items():
    ...
    tabla_accesorios.drawOn(c, 50, 150)  # Ajusta la posición según tu diseño

    # ---- Estado aparte de la tabla ----
    c.setFont("Helvetica-Bold", 10)
    c.drawString(50, 280, f"Estado del equipo: {nuevo.get('Estado', '')}")

    # ---- RITM aparte de la tabla ----
    c.setFont("Helvetica-Bold", 10)
    c.drawString(300, 280, f"Ticket Refresh: {nuevo.get('RITM', 'N/A')}")

    # ---- Firmas debajo del estado ----
    firmas_y = 100
